...
const masalaItems = [
  ...
  { name: 'पंचबगार', quantity: 15, unit: 'gm' },
  { name: 'इलायची', quantity: 15, unit: 'gm' },
  { name: 'जायफल + जावित्री', quantity: 20, unit: 'gm' },
  ...
  { name: 'पाना मेथी', quantity: 30, unit: 'gm' },
  { name: 'अजवाइन', quantity: 30, unit: 'gm' },
  { name: 'साबुत धाना', quantity: 30, unit: 'gm' },
...
const ghareluSaman = [
  ...
  { name: 'खाली कार्टून', quantity: 1, unit: '' },
];
...
const tentItems = [
  ...
  { name: 'गैस भट्टी', quantity: 1, unit: '' },
  { name: 'रोटी भट्टी', quantity: 1, unit: '' },
  ...
  { name: 'चाशनी कढ़ाई', quantity: 1, unit: '' },
  { name: 'बर्तन टब', quantity: 1, unit: '' },
  { name: 'जलेबी तवी', quantity: 1, unit: '' },
  { name: 'छोटा टोपिया', quantity: 1, unit: '' },
  { name: 'बाल्टी', quantity: 1, unit: '' },
  { name: 'जग', quantity: 1, unit: '' },
  { name: 'धामा', quantity: 3, unit: '' },
  { name: 'कुरछा', quantity: 1, unit: '' },
...
const convertToLiters = (quantity, unit) => {
  if (!quantity || isNaN(quantity) || !unit) {
    console.warn(`Invalid quantity or unit: quantity=${quantity}, unit=${unit}`);
    return 0;
  }
  ...
  const unitMap = {
    ...
    'mitar': 1,
    'ml': 0.001,
    'kg': 1, // Assuming 1 kg of oil/ghee ≈ 1 liter (density ~1)
    'gm': 0.001,
    'gram': 0.001,
  };
  const conversionFactor = unitMap[unitLower];
  if (!conversionFactor) {
    console.warn(`Unknown unit for conversion: ${unit}`);
    return 0;
  }
  const result = Number((quantity * conversionFactor).toFixed(3));
  console.log(`Converting ${quantity} ${unit} to ${result} liters`);
  return result;
};
...
const calculateTotals = (items, numberOfPersons, masalaItems) => {
  ...
  (masalaItems || []).forEach((item) => {
    if (!item.name || !item.quantity || !item.unit) {
      console.warn(`Skipping invalid masala item: ${JSON.stringify(item)}`);
      return;
    }
  ...
  console.log(`Final Total Oil: ${totalOil.toFixed(2)} liters, Total Ghee: ${totalGhee.toFixed(2)} liters`);
