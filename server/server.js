...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (!origin || [
      'https://shivshakticatering.netlify.app',
      'https://shivshakticatering.netlify.app/',
...
app.options('/api/compose-latex', (req, res) => {
  console.log('Received OPTIONS request for /api/compose-latex');
  res.set({
...
app.use((req, res, next) => {
  console.log(`Received ${req.method} request at ${req.url} from origin: ${req.get('Origin') || 'unknown'}`);
  next();
});
...
// ... (Cloudinary config, masalaItems, disposableItems, ghareluSaman, tentItems unchanged)
cloudinary.config({
  cloud_name: process.env.CLOUDINARY_CLOUD_NAME,
  api_key: process.env.CLOUDINARY_API_KEY,
  api_secret: process.env.CLOUDINARY_API_SECRET,
...
// ... (formatUnit, formatQuantity, capitalizeText, truncateText, convertToLiters, calculateTotals, isHindi, drawDecorativeBorder, renderTable unchanged)
...
app.post('/api/compose-latex', async (req, res) => {
  try {
    ...
    const { order, withIngredients, filename } = req.body;
    if (!order || !filename) {
      console.error('Missing order or filename');
      return res.status(400).json({ error: 'Missing order or filename' });
    }

    const { orderId, customerName, numberOfPersons, functionDate, functionName, items } = order;
    console.log('Order details:', { orderId, customerName, numberOfPersons, functionDate, functionName });
    ...
    try {
      ...
      if (date && !isNaN(date.getTime())) {
        ...
        console.warn('Invalid date parsed:', functionDate);
      }
    } catch (error) {
      console.error('Error parsing functionDate:', error.message, 'Value:', functionDate);
    }
    ...
    try {
      ...
      console.error('Failed to register fonts:', error.message);
      return res.status(500).json({ error: 'Font registration failed', details: error.message });
    }
    ...
    const checkPageBreak = (currentY, requiredSpace, debugLabel = '') => {
      ...
      if (currentY + requiredSpace > pageHeight) {
        pageBreakCount++;
        console.log(`Adding page break #${pageBreakCount} at y=${currentY}, requiredSpace=${requiredSpace}, pageHeight=${pageHeight}, label=${debugLabel}`);
        if (pageBreakCount > 50) {
          console.error('Too many page breaks, possible recursion');
          throw new Error('Too many page breaks, possible recursion');
        }
        doc.addPage();
        addHeader();
        drawDecorativeBorder(doc);
    ...
    // ... (addCoverPage, addHeader, addSummaryPage unchanged)
    ...
    writeStream.on('finish', () => {
      ...
      fs.readFile(pdfFile, (err, data) => {
        if (err) {
          ...
          return res.status(500).json({ error: 'Failed to read PDF', details: err.message });
        }
        res.set({
          'Content-Type': 'application/pdf',
          'Content-Disposition': `attachment; filename="${filename}.pdf"`,
        ...
        fs.unlink(pdfFile, (err) => {
          ...
          else console.log('Deleted temp PDF file:', pdfFile);
        });
      });
    });
...
const generatePDF = async (order, withIngredients = false) => {
  try {
    ...
    if (error.response) {
      ...
      const errorText = await error.response.data.text().catch(() => 'Could not parse error response');
