...
const HalwaiRegister = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'halwais', userCredential.user.uid), {
        name: data.name,
        email: data.email,
        mobile: data.mobile,
        role: 'halwai',
      });
      navigate('/halwai/dashboard');
    } catch (error) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'halwais', userCredential.user.uid), {
        name: userCredential.user.displayName || 'Halwai',
