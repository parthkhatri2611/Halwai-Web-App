...
//               <ListItemText
//                 primary={notif.message}
//                 secondary={`Status: ${notif.status} | ${notif.createdAt?.toDate ? new Date(notif.createdAt.toDate()).toLocaleString() : new Date(notif.createdAt).toLocaleString()}`}
//               />
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map(notif => (
              ...
              >
                <ListItemText
                  primary={notif.message}
                  secondary={`Status: ${notif.status} | ${notif.createdAt ? new Date(notif.createdAt.toDate ? notif.createdAt.toDate() : notif.createdAt).toLocaleString() : 'N/A'}`}
                />
                {!notif.read && (
                  <IconButton onClick={() => handleMarkAsRead(notif.id)}>
                    <Notifications color="primary" />
